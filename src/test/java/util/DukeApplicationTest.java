...
public class DukeApplicationTest extends ApplicationTest {
  ...
  protected void clickOn(GridPane g, Node item) {
    ...
    simulateAction(offset.getX(), offset.getY(),
        () -> g.getOnMouseClicked().handle(
            new MouseEvent(MouseEvent.MOUSE_CLICKED, offset.getX(), offset.getY(), offset.getX(),
                offset.getY(), MouseButton.PRIMARY, 1,
  ...
  /**
   * Run given view method as a JFX action, allowing you to add/remove components from the Scene or
   * Stage.
   */
