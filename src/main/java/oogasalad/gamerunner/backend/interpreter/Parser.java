...
public class Parser {
  ...
  private ExpressionToken parseBracketToken(BracketToken b, Environment env) {
    if (!b.isOpen()) {
      throw new InvalidSyntaxException("Unexpected closing bracket");
    }
  ...
  private OperatorToken parseOperator(Environment env, OperatorToken m) {
    ...
    for (int i = 0; i < args.length; i++) {
      if (!hasNext()) {
        throw new InvalidSyntaxException("Not enough arguments for operator " + m.SUBTYPE);
      }
  ...
  private SetToken parseSet(ParenthesisToken p, Environment env) {
    if (!p.isOpen()) {
      throw new InvalidSyntaxException("Unexpected closing parenthesis");
    }
    if (!hasNext()) {
      throw new InvalidSyntaxException("Set not closed");
    }
    Token t = next();
    ...
    if (t instanceof OperatorToken op) {
      ...
      if (op.getNumArgs() == 0) {
        throw new InvalidSyntaxException(
            "Set called with operator " + op + " that takes zero arguments");
      }
  ...
  private void createExpression(ExpressionToken expr, Environment env) {
    ...
    if (openBrackets > 0) {
      throw new InvalidSyntaxException("Bracket not closed: " + openBrackets);
    }
  }
