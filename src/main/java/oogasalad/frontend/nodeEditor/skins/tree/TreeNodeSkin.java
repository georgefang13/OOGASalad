...
public class TreeNodeSkin extends GNodeSkin {
  ...
  private static final PseudoClass PSEUDO_CLASS_SELECTED = PseudoClass.getPseudoClass(
      "selected"); //$NON-NLS-1$
...
//        getRoot().getChildren().add(addChildButton);
  ...
  /**
   * Adds a child node with one input and one output connector, placed directly underneath its
   * parent.
   */
  private void addChildNode() {
    ...
    command.append(
        AddCommand.create(editingDomain, parentOutput, CONNECTOR_CONNECTIONS, connection));
   ...
   *
   * <p>
   * Assumes the node has 1 or 2 connectors, and if there are 2 connectors the second is the output.
   * Bit dodgy but only used in the demo.
   * </p>
   */
  ...
  protected Button closeButton() {
    ...
    closeButton.setOnAction(
        event -> Commands.removeNode(getGraphEditor().getModel(), this.getItem()));
  ...
  private void handleFormatting() {
    ...
    background.widthProperty()
        .bind(border.widthProperty().subtract(border.strokeWidthProperty().multiply(2)));
    background.heightProperty()
        .bind(border.heightProperty().subtract(border.strokeWidthProperty().multiply(2)));
