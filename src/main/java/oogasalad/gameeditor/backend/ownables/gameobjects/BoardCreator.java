    ...
    public static List<DropZone> createGrid(int rows, int cols) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                ...
                if (i > 0) z.addOutgoingConnection(nodes[i - 1][j], "Up");
                if (i < rows - 1) z.addOutgoingConnection(nodes[i + 1][j], "Down");
                if (j > 0) z.addOutgoingConnection(nodes[i][j - 1], "Left");
                if (j < cols - 1) z.addOutgoingConnection(nodes[i][j + 1], "Right");
                if (i > 0 && j > 0) z.addOutgoingConnection(nodes[i - 1][j - 1], "UpLeft");
                if (i > 0 && j < cols - 1) z.addOutgoingConnection(nodes[i - 1][j + 1], "UpRight");
                if (i < rows - 1 && j > 0) z.addOutgoingConnection(nodes[i + 1][j - 1], "DownLeft");
                if (i < rows - 1 && j < cols - 1) z.addOutgoingConnection(nodes[i + 1][j + 1], "DownRight");
            }
        }
